...
Pour commencer, assurez-vus d'avoir les trois projets ```dsapitech```, ```react-dsapitech``` et ```projet-test``` sur votre poste.
...
### 5.1 Modification de style

Pour effectuer une modification pour laquelle du CSS suffit, inutile d'utiliser tout le système de dépendances : une édition du fichier ```dsapitech.css``` du repo ```dsapitech``` suffit.

Ce fichier est appelé par les projets, donc les modifications de ce fichier sur la branche main du repo ```dsapitech``` sera immédiatement effective en production.

### 5.2 Autre modification
...
##### Exemple avec une édition de style
